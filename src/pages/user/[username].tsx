    ...
    <table className="table-auto space-x-2">
      ...
      <tbody>
        {props.users.map((user) => {
          return (
            <tr className="hover:bg-white hover:bg-opacity-10" key={user.id}>
              <td>
                {`${user.addedAt.getFullYear().toLocaleString("en-US", {
                  minimumIntegerDigits: 4,
                  useGrouping: false,
                })}-${(user.addedAt.getMonth() + 1).toLocaleString("en-US", {
                  minimumIntegerDigits: 2,
                  useGrouping: false,
                })}-${user.addedAt.getDate().toLocaleString("en-US", {
                  minimumIntegerDigits: 2,
                  useGrouping: false,
                })}`}
              </td>
...
const Followers = (props: { userId: string }) => {
  ...
  return (
    <div className="rounded border border-gray-600 bg-gray-800 bg-opacity-10 p-4">
  ...
  return (
    <main className="container mx-auto flex min-h-screen flex-col items-center py-16 ">
        ...
        <div className="container mx-auto flex max-w-xl flex-col rounded border border-gray-600 bg-gray-800 bg-opacity-10 p-4">
          {metadata.profile_image_url && (
            <img
              ...
              src={metadata.profile_image_url.replace("normal", "200x200")}
            ></img>
          )}
            ...
            <div className="flex flex-row space-x-2">
              <div>
                <span className="font-bold">
                  {metrics.following_count?.toLocaleString(undefined, {
                    notation: "compact",
                  })}
                </span>{" "}
                <span className="font-light opacity-80">Following</span>
              </div>
              <div>
                <span className="font-bold">
                  {metrics.followers_count?.toLocaleString(undefined, {
                    notation: "compact",
                  })}
                </span>{" "}
                <span className="font-light opacity-80">Followers</span>
              </div>
              <div>
                <span className="font-bold">
                  {metrics.tweet_count?.toLocaleString(undefined, {
                    notation: "compact",
                  })}
                </span>{" "}
                <span className="font-light opacity-80">Tweets</span>
              </div>
            </div>
      ...
      <div className="grid grid-cols-3 gap-4">
        {query.data?.id && <Following userId={query.data?.id}></Following>}
        {query.data?.id && <Followers userId={query.data?.id}></Followers>}
