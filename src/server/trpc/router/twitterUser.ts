...
import { z } from "zod";
import { getAllPopularUserDynamic } from "@acme/db/queries/types/popular_user";
import {
...
} from "@acme/db/queries/types/trending_user";
    ...
    getById: publicProcedure
        ...
        .query(async ({ ctx, input }) => {
            ...
            return {
                ...
                public_metrics:
                    user.twitterPublicMetrics ?? ({} as UserPublicMetrics),
            } as User;
        }),
                ...
                const result = await getAllPopularUserDynamic.run(
                    {
                        ...
                        weighted_marked_followers_min: safeParseInt(
                            input.filters.find(
                                (x) => x.id === "weighted_marked_followers"
                            )?.value?.[0]
                        ),
                        ...
                        listed_count_min: safeParseInt(
                            input.filters.find((x) => x.id === "listed_count")
                                ?.value?.[0]
                        ),
                        ...
                        tweet_count_min: safeParseInt(
                            input.filters.find((x) => x.id === "tweet_count")
                                ?.value?.[0]
                        ),
                        ...
                        sort_column:
                            input.sorting?.[0]?.id ?? "marked_followers",
                ...
                const result = await query.run(
                    {
                        difference_min: safeParseInt(
                            input.filters.find((x) => x.id === "difference")
                                ?.value?.[0]
                        ),
